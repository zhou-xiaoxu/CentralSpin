...
class CentralSpin(object):
    ...
    def fid(self, state_list):
        ...
        fid.append([(s[0] * self.c_tar).tr() + 2 * np.sqrt(np.linalg.det(s[0]) * np.linalg.det(self.c_tar)) for s in state_list[0]])
        for i in range(1,self.N+1):
            fid.append([(s[0] * self.env_tar).tr() + 2 * np.sqrt(np.linalg.det(s[0]) * np.linalg.det(self.env_tar)) for s in state_list[i]])
        
        fid2 = []  # general form of fidelity
        fid2.append([np.square(np.absolute((self.c_tar.sqrtm() * s[0] * self.c_tar.sqrtm()).sqrtm().tr())) for s in state_list[0]])
        for i in range(1,self.N+1):
            fid2.append([np.square(np.absolute((self.env_tar.sqrtm() * s[0] * self.env_tar.sqrtm()).sqrtm().tr())) for s in state_list[i]])
